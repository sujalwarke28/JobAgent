  ...
  return (
    <div className="max-w-5xl mx-auto pb-12 animate-in fade-in duration-700 slide-in-from-bottom-4">
      <div className="mb-10 text-center md:text-left relative">
        ...
        <h1 className="text-4xl font-extrabold text-white mb-3 tracking-tight">Application <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500">Journey</span></h1>
        <p className="text-gray-400 text-lg">Track and manage your AI-assisted career opportunities</p>
      </div>

      {applications.length === 0 ? (
        <div className="relative overflow-hidden bg-gray-800/40 backdrop-blur-sm border border-gray-700/50 rounded-3xl p-16 text-center shadow-2xl transition-transform hover:scale-[1.01] duration-500">
          <div className="absolute inset-0 bg-gradient-to-br from-blue-500/5 to-purple-500/5"></div>
          <FileText strokeWidth={1} className="w-24 h-24 text-gray-600 mx-auto mb-6 relative z-10" />
          <h3 className="text-2xl font-semibold text-white mb-3 relative z-10">No adventures started</h3>
          <p className="text-gray-400 mb-8 max-w-md mx-auto relative z-10 text-lg">Your next great opportunity is out there. Let AI find your perfect match.</p>
          <Link to="/dashboard" className="relative z-10 inline-flex items-center gap-2 px-8 py-3.5 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white rounded-xl transition-all shadow-lg hover:shadow-blue-500/25 font-medium group text-lg">
        ...
        <div className="grid gap-6">
          {applications.map((app, idx) => {
            ...
            return (
              <div 
                key={app._id} 
                className="group relative bg-gray-800/40 backdrop-blur-md border border-gray-700/50 rounded-2xl p-1 md:p-1.5 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:border-gray-600/50"
                style={{ animationDelay: `${idx * 100}ms` }}
              >
                <div className="bg-gray-900 rounded-xl p-6 relative overflow-hidden h-full flex flex-col md:flex-row gap-6 items-start md:items-center">
                  ...
                  <div className="flex-grow z-10">
                    <div className="flex items-center justify-between md:justify-start gap-4 mb-3">
                      <h3 className="text-2xl font-bold text-white tracking-tight">{app.jobId.title}</h3>
                    ...
                    <div className="flex flex-wrap gap-x-6 gap-y-3 text-sm text-gray-400 font-medium">
                      <span className="flex items-center gap-1.5 bg-gray-800/80 px-3 py-1.5 rounded-lg border border-gray-700/50"><Calendar className="w-4 h-4 text-gray-500"/> {new Date(app.appliedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                      <span className="flex items-center gap-1.5 bg-gray-800/80 px-3 py-1.5 rounded-lg border border-gray-700/50"><MapPin className="w-4 h-4 text-gray-500"/> {app.jobId.location}</span>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto mt-2 md:mt-0 z-10 Shrink-0">
                    ...
                    <Link 
                      ...
                      className="px-6 py-3 bg-gray-800 hover:bg-gray-700 border border-gray-600 text-white rounded-xl flex items-center justify-center gap-2 text-sm font-bold transition-all hover:border-gray-500 hover:shadow-lg"
                    >
                      <span>Full Details</span> <ExternalLink className="w-4 h-4 text-gray-400" />
                    </Link>
                  </div>
